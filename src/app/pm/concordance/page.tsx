...
export default async function PmConcordancePage({
  searchParams,
}: {
  ...
  return (
    <>
      <PageHeader
        title="Concordance"
        subtitle="Search across every translation memory."
      />
      ...
      {query.length >= 2 ? (
        results.length === 0 ? (
          <div className="rounded-lg border border-dashed border-[color:var(--color-border)] p-6 text-sm text-[color:var(--color-slate-500)] italic">
            No matches for &ldquo;{query}&rdquo; in any translation memory.
          </div>
        ) : (
        ...
        <div className="rounded-lg border border-dashed border-[color:var(--color-border)] p-6 text-sm text-[color:var(--color-slate-500)] italic">
          ...
          translations across all translation memories.
        </div>
      )}
    </>
  );
