...
import { exportJsonBuffer, type JsonPath } from "@/lib/jobs/json-extraction";
...
export async function GET(req: Request, ctx: { params: Promise<{ jobId: string }> }) {
  ...
  if (job.source_format !== "json") {
    return NextResponse.json(
      { error: `Cannot export JSON from a ${job.source_format} source` },
      { status: 400 },
    );
  }
  ...
  const translatedSegments = (segs as Array<{
    ...
    .map((s) => {
      const loc = (s.meta as Record<string, unknown>).location as { kind: string; path: JsonPath };
      return {
        source_text: s.source_text,
        target_text: s.target_text,
        location: { kind: "json" as const, path: loc.path },
      };
    });

  const buffer = exportJsonBuffer(sourceBuffer, translatedSegments);

  const baseName = job.source_filename.replace(/\.json$/i, "");
  const filename = `${baseName}-${job.target_lang}.json`;
