  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Link href="/admin/integrations/portal" className="bg-white rounded-xl border border-[color:var(--color-border)] p-8 hover:bg-[color:var(--color-slate-50)] block">
          <div className="font-semibold text-[color:var(--color-navy)]">Portal sync (clients + vendors)</div>
          <div className="text-sm text-[color:var(--color-slate-500)] mt-1">Pull customers and vendors from the shared admin/vendor-portal Supabase.</div>
          <div className="text-xs text-[color:var(--color-teal-700)] font-semibold mt-2">Open sync →</div>
        </Link>
        <PlaceholderCard title="Vendor Portal SSO" body="Configure JWT issuer, JWKS URL, audience." />
        <Link href="/admin/integrations/api-keys" className="bg-white rounded-xl border border-[color:var(--color-border)] p-8 hover:bg-[color:var(--color-slate-50)] block">
